<script lang="ts">
  ...
  import { createVaultSecret, deleteVaultSecret, deleteVaultSecretPolicy, fetchVaultSecretPolicies, fetchVaultSecrets, fetchVaultStatus, fetchWorkspace, initVault, lockVault, unlockVault, updateVaultSecret, upsertVaultSecretPolicy } from '$lib/nucleus/client';
  import type { ProjectSummary, VaultSecretPolicySummary, VaultSecretSummary, VaultStatusSummary } from '$lib/nucleus/schemas';
  ...
  async function loadVault() {
    ...
    try {
      ...
      if (status.initialized && canLoadScope) {
        ...
        for (const secret of secrets) next[secret.id] = (await fetchVaultSecretPolicies(secret.id, { scope_kind: secret.scope_kind, scope_id: secret.scope_id })).policies;
        policies = next;
      } else {
        secrets = [];
        policies = {};
  ...
  async function saveSecret() { saving = true; try { if (!canLoadScope) throw new Error('Select a project before saving a project Vault secret.'); if (editingSecretId) { await updateVaultSecret(editingSecretId, { ...secretForm, scope_kind: selectedScopeKind, scope_id: activeScopeId }); success = 'Vault secret replaced. Plaintext was not returned or displayed.'; } else { await createVaultSecret({ ...secretForm, scope_kind: selectedScopeKind, scope_id: activeScopeId }); success = 'Vault secret created. Plaintext was not returned or displayed.'; } resetSecretForm(); await loadVault(); } catch (cause) { error = cause instanceof Error ? cause.message : 'Failed to save Vault secret.'; } finally { saving = false; } }
  ...
  async function savePolicy() { if (!policySecretId) return; const secret = secrets.find((item) => item.id === policySecretId); await upsertVaultSecretPolicy(policySecretId, policyForm, secret ? { scope_kind: secret.scope_kind, scope_id: secret.scope_id } : { scope_kind: selectedScopeKind, scope_id: activeScopeId }); success = 'Updated Vault access policy metadata.'; await loadVault(); }
  async function removePolicy(policy: VaultSecretPolicySummary) { const secret = secrets.find((item) => item.id === policy.secret_id); await deleteVaultSecretPolicy(policy.secret_id, policy.id, secret ? { scope_kind: secret.scope_kind, scope_id: secret.scope_id } : { scope_kind: selectedScopeKind, scope_id: activeScopeId }); success = 'Deleted Vault access policy metadata.'; await loadVault(); }
  async function copyReference(secret: VaultSecretSummary) { const reference = secret.scope_kind === 'project' ? `vault://project/${secret.scope_id}/${secret.name}` : `vault://workspace/${secret.name}`; await navigator.clipboard.writeText(reference); success = `Copied reference for ${secret.name}.`; }
  function switchScope(kind: 'workspace' | 'project') { selectedScopeKind = kind; resetSecretForm(); void loadVault(); }
  function switchProject(projectId: string) { selectedProjectId = projectId; resetSecretForm(); void loadVault(); }
