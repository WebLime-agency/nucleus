...
use clap::Parser;
use nucleus_approval_policy::{
    ApprovalVerdict, command_looks_like_validation, command_session_looks_like_check_validation,
    command_text, evaluate_harness_approval,
    tool_call_command_text as policy_tool_call_command_text, tool_name_is_publication,
};
#[cfg(test)]
use nucleus_approval_policy::{
    command_has_shell_escape_or_write, command_looks_like_read_build_or_test,
    network_policy_allows_network, normalize_command_run_like_args, normalize_tests_run_like_args,
};
...
fn evaluate_approval(approval: &ApprovalRequestSummary, job: &JobDetail) -> ApprovalVerdict {
    let Some(call) = job
        ...
        return ApprovalVerdict {
            approve: false,
            reason: "tool call for approval was not found".to_string(),
    ...
    let Some(worker) = job
        ...
        return ApprovalVerdict {
            approve: false,
            reason: "worker for approval was not found".to_string(),
            target_paths: Vec::new(),
        };
    };
    ...
    if worker.working_dir.trim().is_empty() || !worktree.is_absolute() {
        return ApprovalVerdict {
            approve: false,
            reason: "worker working_dir is missing or not absolute".to_string(),
            target_paths: Vec::new(),
        };
    }

    evaluate_harness_approval(&call.tool_id, &call.args_json, worktree)
}

#[cfg(test)]
fn evaluate_command_run(call: &ToolCallSummary, worktree: &Path) -> ApprovalVerdict {
    evaluate_harness_approval("command.run", &call.args_json, worktree)
}

#[cfg(test)]
fn evaluate_python_run(call: &ToolCallSummary, worktree: &Path) -> ApprovalVerdict {
    evaluate_harness_approval("python.run", &call.args_json, worktree)
}

#[cfg(test)]
fn evaluate_tests_run(call: &ToolCallSummary, worktree: &Path) -> ApprovalVerdict {
    evaluate_harness_approval("tests.run", &call.args_json, worktree)
}
...
fn tool_call_command_text(call: &ToolCallSummary) -> String {
    policy_tool_call_command_text(&call.tool_id, &call.args_json)
}
